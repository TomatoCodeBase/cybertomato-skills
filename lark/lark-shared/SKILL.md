...
`npm install -g` 的 postinstall 脚本可能因网络或权限原因未能下载 lark-cli 二进制文件，报错：
```
Error: lark-cli binary not found at D:\...\node_modules\@larksuite\cli\bin\lark-cli.exe
```

**解决方案**：手动下载二进制文件。国内网络优先用 npmmirror（秒下），GitHub Releases 极慢：

```bash
# 1. 查看 npm 安装的版本号
npm list -g @larksuite/cli

# 2. 从 npmmirror 下载（替换 VERSION 为实际版本号）
VERSION="1.0.7"
curl --ssl-revoke-best-effort --fail --location --max-time 300 \
  --output /tmp/lark-cli.zip \
  "https://registry.npmmirror.com/-/binary/lark-cli/v${VERSION}/lark-cli-${VERSION}-windows-amd64.zip"

# 3. 解压并复制到 npm 期望的位置
cd /tmp && unzip -o lark-cli.zip
cp /tmp/lark-cli.exe "$(npm root -g)/@larksuite/cli/bin/lark-cli.exe"

# 4. 验证
lark-cli --version
```

**Linux/macOS** 同理，替换 `windows-amd64.zip` 为 `darwin-arm64.tar.gz` / `linux-amd64.tar.gz`。
...
npx skills add larksuite/cli -y -g
# 升级后手动下载二进制文件（如上述步骤）
# 然后将最新 Skills 同步到 Hermes：
rm -rf ~/.hermes/skills/lark/lark-*
cp -r ~/.agents/skills/lark-* ~/.hermes/skills/lark/
```

### Agent 代理发起认证（设备码流程）

当 `auth login` 交互式阻塞时，使用设备码流程：

```bash
# 步骤 1：获取验证链接（立即返回，不阻塞）
lark-cli auth login --recommend --no-wait
# 输出 JSON 含 verification_url 和 device_code

# 步骤 2：把 verification_url 和 user_code 发给用户，让用户在浏览器中授权

# 步骤 3：用户授权完成后，用 device-code 完成登录
lark-cli auth login --device-code "<DEVICE_CODE>"
```

**注意**：device_code 有效期 600 秒（10分钟），过期需重新执行步骤 1。
