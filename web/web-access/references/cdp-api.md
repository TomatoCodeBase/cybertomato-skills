...
- **MINGW64/PowerShell 中文编码陷阱（2026-04-13）**：`curl -d '中文'` 在 MINGW64/Git Bash 和 PowerShell 下都会损坏 UTF-8 编码，`--data-binary` 也无法解决。**必须**将含中文的 JS 代码写入 `.js` 文件（UTF-8），用 `curl -d @/tmp/xxx.js` 从文件读取传递。这对 `/eval` 和 `/click`（POST body 含中文选择器时）都适用
