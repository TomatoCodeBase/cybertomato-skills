...
### 大文档+图片会触发异步模式（无 polling 机制）

`+create` 传入大量 markdown 内容（尤其是含多张 `<image>` 标签）时，可能返回异步任务：

```json
{"status": "running", "task_id": "xxx", "message": "Task is processing in the background..."}
```

**问题**：lark-cli 没有提供 polling 机制——`+create` 没有 `--task-id` 参数，`drive +task_result` 也不适用于 doc 创建任务。文档可能创建成功但无法获取 doc_id 和 URL。

**可靠方案**：分两步走——先同步创建空文档，再 overwrite 写入完整内容：

```bash
# Step 1: 同步创建（极简内容，确保拿到 doc_id）
lark-cli docs +create --title "我的文档" --markdown "placeholder" --as user
# → 返回 doc_id 和 doc_url

# Step 2: overwrite 写入完整内容（支持 @file 方式）
cd /tmp && lark-cli docs +update --doc "DOC_ID" --mode overwrite --markdown @full_content.md --as user
```

### Windows @file 路径限制

`--markdown @file` 只接受**相对路径**（当前目录内）。绝对路径如 `@/tmp/xxx.md` 或 `@C:\Users\...\xxx.md` 会报错。

**解决**：先 `cd` 到文件所在目录，再用相对路径：
```bash
cd /tmp && lark-cli docs +create --title "xxx" --markdown @content.md --as user
```

### 图片下载失败是 warning 不阻塞

部分外部图床 URL（如 imgdb.cn）可能被飞书服务器无法访问，返回 `IMAGE_DOWNLOAD_FAILED` warning，但文档照常创建成功。需要事后手动检查并补图。
...
- **目录**：飞书自动生成，无需手动添加
- **大文档策略**：先 create 同步拿 doc_id，再 overwrite 写完整内容，避免异步模式丢结果
